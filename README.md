...
Setup
Create a new empty repository on GitHub (no README/template), e.g.
`ka-agency-contract`.
In this folder, run:
```bash
   git init
   ...
   ```
Go to vercel.com, import the new GitHub repo as a
...
Resend account.
Redeploy after adding the env var so it takes effect.
That's it — the deployed URL is what you send to a client to sign.
Notes
Emails only reliably deliver to `kzstech000@gmail.com` while you're on
